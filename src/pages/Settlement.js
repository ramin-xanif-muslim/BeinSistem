...
export default function Settlement() {
  ...
  const { visibleDrawer, setVisibleDrawer, setcusid, cusid } = useCustomForm();
  ...
  const columns = useMemo(() => {
    return [
...

      {
        ...
        title: "Borc (alacaq)",
        show: initial
          ? Object.values(initial).find((i) => i.dataIndex === "PayIn").show
          : true,
      ...
      },
      {
        ...
        title: "Borc (verəcək)",
        show: initial
          ? Object.values(initial).find((i) => i.dataIndex === "PayOut").show
          : true,
        render: (value, row, index) => {
          if (row.Amount < 0) {
            return row.Amount;
      ...
      },
      {
        ...
        title: "Cari vəziyyət",
        show: initial
          ? Object.values(initial).find((i) => i.dataIndex === "Current").show
          : false,
        render: (value, row, index) => {
          return row.Amount;
        },
  ...
  useEffect(() => {
    if (!isFetching) {
      ...
      setallcurrentsum(parseFloat(data.Body.AllInSum + data.Body.AllOutSum));
    } else {
      setDocumentList([]);
    }
  }, [isFetching]);
